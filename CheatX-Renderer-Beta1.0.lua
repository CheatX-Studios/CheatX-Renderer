...
lib.createWindow = function(name, title, draggable)
	...
	WindowLib.createTab = function(name)
		...
		if not default_tab then
			default_tab = TabButton.Name
		else
		...
		TabLib.createSection = function(text, isopen)
			...
			Section.ChildAdded:Connect(function(child)
				if child.Name == "CheatXRSectionText" or child.Name == "UIListLayout" then
				else
			...
			SectionText.Name = "CheatXRSectionText"
			...
			OpenCloseIcon.MouseButton1Down:Connect(function()
				...
				for _,v in pairs(Section:GetChildren()) do
					if v.Name == "CheatXRSectionText" or v.Name == "UIListLayout" then
					else
						v.Visible = isdown
					end
				end
	...
	if isfile('CheatXRLibVer.txt') then
		if readfile('CheatXRLibVer.txt') == ver then
		else
			print(changelog)
			writefile('CheatXRLibVer.txt', ver)
			WindowLib.notification("CheatXR | New Version", changelog)
		end
	else
		writefile('CheatXRLibVer.txt', ver)
		WindowLib.notification("CheatXR | Welcome", "This seems to be your first time using CheatXR Library !")
		setclipboard('')
	end
